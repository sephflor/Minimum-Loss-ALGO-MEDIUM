...
class Result
{
    ...
    public static int minimumLoss(List<long> price)
    {
        ...
        for (int i = 1; i < indexedPrices.Count; i++)
        {
            long diff = indexedPrices[i].Price - indexedPrices[i - 1].Price;
            
            
            if (indexedPrices[i].Index < indexedPrices[i - 1].Index && diff < minLoss)
            {
                minLoss = diff;
            }
        }
